...
export async function GET(request: NextRequest): Promise<NextResponse> {
	try {
		...
		// Determine the available years (local time) across all data, so the UI
		// can offer an "all-time" view plus a per-year filter.
		...
		// Filter the dataset to the selected scope (all-time vs a single year).
		const reservations = isYearFilter
			? allReservations.filter(
					(r) =>
						getNLParts(new Date(r.reservation_start)).year === selectedYear
				)
			: allReservations
		...
		if (reservations.length === 0) {
			return NextResponse.json({
				...
				availableYears,
				selectedYear: isYearFilter ? String(selectedYear) : 'all'
			})
		}
		...
		for (const reservation of reservations) {
			...
			// Bucket the rental by its (local) start day / month.
			...
			// Weekday of the (local) start day.
		...
		// --- Build a continuous list of days across the full period --------
		...
		// Count days per month and accumulate available hours.
