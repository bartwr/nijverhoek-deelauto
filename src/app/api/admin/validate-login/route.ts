...
export async function POST(
	...
	try {
		...
		const loginToken = await db.collection('LoginTokens').findOne({
			token,
			used: false,
		...
		await db.collection('LoginTokens').updateOne(
			{ _id: loginToken._id },
			{ $set: { used: true } }
		)
		...
		await db.collection('AdminSessions').insertOne(adminSession)
